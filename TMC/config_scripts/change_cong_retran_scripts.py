...
def change_TCP_retransmission (re):
	...
	#	os.system ('echo BARR-PRO > /proc/sys/net/ipv4/tcp_retransmission_algorithm')
...
for cong in range (4):
	...
	for re in range (3):
		...
		#for var_sysctl in range(1,8,2):
		#	os.system ('echo %d > /proc/sys/net/ipv4/tcp_temp_threshold'%var_sysctl)
		#	os.system ('echo %d > /proc/sys/net/ipv4/tcp_queue_threshold'%(var_sysctl + 1))
		for time in range (4):
			#print ("this is %d test"%time)
			os.system ('iperf -c 192.168.123.212 -t 100 > test_data.txt')
